...
export function ReservationPanel() {
    ...
    const [reservationItems, setReservationItems] = useState<Array<{ item_id: string; quantity: number | string }>>([{ item_id: '', quantity: 1 }]);
    ...
    const getStatusGroupStyle = (status: string) => {
        switch (status) {
            case 'pending':
                return {
                    headerBg:
                        'bg-gradient-to-r from-yellow-50 to-orange-50 border-yellow-300 dark:from-yellow-950/30 dark:to-orange-950/30 dark:border-yellow-700',
            ...
            for (const item of validItems) {
                ...
                if (stockPercentage > 50 && inventoryItem.stock > 1) {
                    validationErrors.push(
                        `${inventoryItem.item_name}: Reserving ${item.quantity} of ${inventoryItem.stock} items (${Math.round(stockPercentage)}% of stock). Consider reducing quantity.`,
                    );
                }
            }
    ...
    const activeReservations = reservations.filter((r: Reservation) => r.status === 'pending' || r.status === 'approved');
    const completedReservations = reservations.filter((r: Reservation) => r.status === 'completed');
                            ...
                            <div className="space-y-4">
                                ...
                                <div>
                                    <div className="mb-2 flex items-center justify-between">
                                        ...
                                        <Button type="button" variant="outline" size="sm" onClick={addReservationItem} className="h-8">
                                    ...
                                    <div className="max-h-64 space-y-3 overflow-y-auto">
                                        {reservationItems.map((item, index) => (
                                            <div
                                                key={`reservation-item-${index}-${item.item_id || 'empty'}`}
                                            ...
                                            >
                                                <div className="flex-1">
                                                    ...
                                                    >
                                                        <SelectTrigger className="h-8 border-border bg-input text-foreground">
                                                            <SelectValue placeholder="Select part">
                                                                {(() => {
                                                                    if (!item.item_id) return 'Select part';
                                                                    const selectedItem = inventoryItems.find(
                                                                        (inv) => String(inv.item_id) === String(item.item_id),
                                                                    );
                                                                    return selectedItem
                                                                        ? `${selectedItem.item_name} (Stock: ${selectedItem.stock})`
                                                                        : 'Select part';
                                                                })()}
                                                            </SelectValue>
                                                        </SelectTrigger>
                                                                    ...
                                                                    .map((inventoryItem) => {
                                                                        ...
                                                                        return (
                                                                            <SelectItem
                                                                                ...
                                                                                className={`${isOutOfStock ? 'cursor-not-allowed opacity-50' : ''} ${isLowStock ? 'text-orange-600' : ''}`}
                                                                            >
                                                                                <div className="flex w-full items-center justify-between">
                                                                                    ...
                                                                                    <span
                                                                                        className={`ml-2 text-xs ${isOutOfStock ? 'text-red-500' : isLowStock ? 'text-orange-500' : 'text-muted-foreground'}`}
                                                                                    >
                                                                                        {isOutOfStock
                                                                                            ? 'Out of Stock'
                                                ...
                                                <div className="w-24">
                                                    ...
                                                    <Input
                                                                ...
                                                                if (selectedItem) {
                                                                    ...
                                                                    if (qty > selectedItem.stock) {
                                                                        setCreateReservationError(
                                                                            `Cannot reserve ${qty} of ${selectedItem.item_name} (only ${selectedItem.stock} in stock)`,
                                                                        );
                                                                    } else {
                                                                        // Clear stock-related errors if quantity is valid
                                                                        if (
                                                                            createReservationError?.includes('only') &&
                                                                            createReservationError?.includes('in stock')
                                                                        ) {
                                                                            setCreateReservationError(null);
                                                                        }
                                                                    }
                                                        ...
                                                        className="h-8 border-border bg-input text-foreground"
                                                    />
                                                </div>
                                ...
                                {createReservationError && (
                                    <Alert variant="destructive">
                                        ...
                                        <AlertDescription>{createReservationError}</AlertDescription>
                                    </Alert>
                                )}
                                ...
                                <div className="flex items-center gap-2">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-sm text-orange-700 dark:text-orange-300">
                                            ...
                                            {reservations.filter((r) => r.status === 'pending').length === 1 ? 's' : ''} immediate attention
                                        </p>
                                    </div>
                                </div>
                            </div>
                ...
                <CardContent>
                    ...
                    <div className="space-y-4">
                        {groupReservationsByStatus(reservations).map(([status, statusReservations]) => {
                            ...
                            return (
                                <div key={status} className="overflow-hidden rounded-lg border border-border">
                                    {/* Status Group Header */}
                                    <Collapsible open={!isCollapsed} onOpenChange={() => toggleGroupCollapse(status)}>
                                        <CollapsibleTrigger className="w-full">
                                            <div
                                                className={`${style.headerBg} border-b border-border p-4 transition-opacity hover:opacity-90 ${status === 'pending' ? 'ring-2 ring-yellow-400/30' : ''}`}
                                            >
                                                <div className="flex items-center justify-between">
                                                    <div className="flex items-center gap-3">
                                                        <div className="flex items-center gap-2">
                                                ...
                                                <Table>
                                                    <TableHeader>
                                                        <TableRow className="border-border bg-muted/30">
                                                            <TableHead className="w-[120px] font-semibold text-foreground">Job Order</TableHead>
                                                            <TableHead className="w-[130px] font-semibold text-foreground">Part Number</TableHead>
                                                            <TableHead className="min-w-[200px] font-semibold text-foreground">Description</TableHead>
                                                            ...
                                                            <TableHead className="w-[120px] font-semibold text-foreground">Requested By</TableHead>
                                                            <TableHead className="w-[100px] text-center font-semibold text-foreground">
                                                                Created
                                                            </TableHead>
                                                            <TableHead className="w-[180px] text-center font-semibold text-foreground">
                                                                Actions
                                                            </TableHead>
                                                        </TableRow>
                                                    </TableHeader>
                                                    <TableBody>
                                                        {statusReservations.map((reservation: Reservation) => {
                                                            ...
                                                            const isUrgent =
                                                                status === 'pending' &&
                                                                new Date().getTime() - new Date(reservation.created_at).getTime() >
