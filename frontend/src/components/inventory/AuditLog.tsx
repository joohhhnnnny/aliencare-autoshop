...
export function AuditLog() {
    ...
    const { transactions, stats, loading, error, refreshing, refresh, setAutoRefresh, isRealTime, lastUpdated } = useAuditLog({
    ...
    const filteredTransactions = transactions.filter((transaction) => {
        const matchesSearch =
            transaction.item_id?.toString().toLowerCase().includes(searchTerm.toLowerCase()) ||
        ...
        return (
            <div className="space-y-6">
                <div className="py-8 text-center">
                    <div className="mb-4 text-red-600">Error loading audit data</div>
