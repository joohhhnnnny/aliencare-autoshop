        ...
        <Card className="mb-6 border-blue-200 bg-blue-50">
            ...
            <CardContent className="space-y-4">
                <p className="text-sm text-blue-700">
                    Use the buttons below to simulate inventory operations and see real-time updates in the audit log. Make sure to enable "Real-time"
                    mode in the audit log panel above.
                </p>
                ...
                <div className="flex flex-wrap gap-3">
                    ...
                    >
                        {isSimulating ? <RefreshCw className="mr-2 h-4 w-4 animate-spin" /> : <Package className="mr-2 h-4 w-4" />}
                    ...
                    >
                        {isSimulating ? <RefreshCw className="mr-2 h-4 w-4 animate-spin" /> : <ShoppingCart className="mr-2 h-4 w-4" />}
                    ...
                    >
                        {isSimulating ? <RefreshCw className="mr-2 h-4 w-4 animate-spin" /> : <Truck className="mr-2 h-4 w-4" />}
                        Simulate Mixed Operations
                    </Button>
                </div>

                {isSimulating && (
                    <div className="animate-pulse text-sm text-blue-600">⚡ Simulating operations... Check the audit log for real-time updates!</div>
                )}
            </CardContent>
        </Card>
    );
